...
1. Όταν ανιχνεύεται νέα έκδοση συστήματος (System Version, π.χ. `1.5.57`), ο Agent κατεβάζει το zip του συστήματος.
2. Δημιουργεί ένα δυναμικό αρχείο δέσμης ενεργειών **`.bat`** στον προσωρινό φάκελο των Windows.
3. Το αρχείο `.bat` εκτελείται με δικαιώματα Administrator (`runas` UAC):
   * Σταματάει την υπηρεσία `TmsAgent` (αν τρέχει).
   * Περιμένει σε βρόχο (loop) μέχρι να ξεκλειδωθεί το εκτελέσιμο `Tms.Agent.Wpf.exe` (μέχρι να κλείσει η τρέχουσα εφαρμογή).
   * Αντιγράφει τα νέα αρχεία στον φάκελο εγκατάστασης του Agent (`C:\Program Files (x86)\TmsAgent`).
