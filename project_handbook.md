...
8. **Κανόνες Client-Role Agents (No Direct DB Connections):**
   ...
   * **Εκτέλεση SQL scripts από τερματικά (Remote SQL Update):** Εάν ένας διαχειριστής επιθυμεί να εκτελεί τις αναβαθμίσεις της βάσης (SQL scripts) απευθείας από το δικό του τερματικό (Client machine) αντί για τον κεντρικό SQL Server, ο Agent του συγκεκριμένου υπολογιστή πρέπει να δηλωθεί ως `"Both"` ή `"SqlServer"` (μέσω Setup Wizard ή Web Console). Σε αυτή την περίπτωση, πρέπει να διασφαλιστεί ότι: (α) Ο Agent έχει δίκτυο προς τον SQL Server (ανοιχτή θύρα 1433), και (β) Έχει ρυθμιστεί η κατάλληλη IP/ConnectionString για το συγκεκριμένο μηχάνημα (μέσω της δυνατότητας «Connection String ανά Μηχάνημα») ώστε να στοχεύει την απομακρυσμένη βάση.
...
19. **Δυναμικό Redirection / Αλλαγή URL σύνδεσης των Agents (Agent Server URL Redirect):**
    * Υποστηρίζεται η δυνατότητα να αλλάξει το URL του διακομιστή (Central Server URL) στο οποίο συνδέονται όλοι οι Agents, χωρίς να απαιτείται χειροκίνητη αλλαγή σε κάθε υπολογιστή ξεχωριστά.
    * Η ρύθμιση ορίζεται κεντρικά στη σελίδα «Ρυθμίσεις Συστήματος» (`/SystemSettings`) ή στο αρχείο `appsettings.json` του Server μέσω του κλειδιού `"AgentRedirectServerUrl": "http://new-domain:5007"`.
    * **Έλεγχος & Προστασία URL (URL Validation):** Στη σελίδα «Ρυθμίσεις Συστήματος», κάθε νέο URL ελέγχεται υποχρεωτικά πριν την αποθήκευση και αποστολή του στους Agents. Εάν το URL δεν έχει έγκυρη σύνταξη ή δεν ανταποκρίνεται ενεργός διακομιστής TMS Central (μέσω reachability check & TMS handshake), η αποθήκευση ακυρώνεται και εμφανίζεται αναλυτικό σφάλμα, προστατεύοντας τους Agents από ακούσια αποσύνδεση. Υπάρχει επίσης κουμπί «🔍 Έλεγχος» για άμεση δοκιμή από το UI.
    * Κατά την επόμενη επικοινωνία (check updates) των Agents (τόσο της υπηρεσίας Service όσο και του WPF GUI Panel), αν διαπιστωθεί διαφορετικό URL από το τρέχον, ο Agent ενημερώνει αυτόματα τις τοπικές του ρυθμίσεις (`settings.json`) και από εκείνο το σημείο και μετά επικοινωνεί αποκλειστικά με το νέο URL.
...
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * **Server:** Προσθήκη μηχανισμού ελέγχου εγκυρότητας και πραγματικής διαθεσιμότητας (connectivity check & TMS handshake) για τα URL ανακατεύθυνσης Agents (Prod & Test) στη σελίδα «Ρυθμίσεις Συστήματος» (`/SystemSettings`) και στη διαχείριση μηχανημάτων (`Clients`).
  * **Server:** Αποτροπή αποθήκευσης και εμφάνιση αναλυτικού μηνύματος σφάλματος σε περίπτωση που το URL δεν είναι έγκυρο ή ο διακομιστής δεν ανταποκρίνεται, προστατεύοντας τους Agents από ακούσια αποσύνδεση.
  * **Server:** Προσθήκη κουμπιού άμεσου ελέγχου σύνδεσης («🔍 Έλεγχος») στη φόρμα Ρυθμίσεων Συστήματος και νέων endpoints ελέγχου υγείας (`/api/updates/ping` και `/api/updates/validate-url`).
  * **Client:** Ενημέρωση της έκδοσης Agent σε 1.5.101 (`AgentVersionInfo.Version`) και αυτόματος συγχρονισμός έκδοσης στο `publish-update.ps1` για αποφυγή επαναλαμβανόμενων λήψεων.

### Έκδοση 1.5.100
* **Ημερομηνία:** 02/09/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Προσθήκη αυτόνομης εντολής `TestServerConnectionCommand` για το κουμπί «Έλεγχος Σύνδεσης» στην καρτέλα Ρυθμίσεις του Agent, εξασφαλίζοντας άμεση δοκιμή επικοινωνίας με τον διακομιστή και έλεγχο εγκυρότητας του URL και του API Key.
  * Τοποθέτηση banner ειδοποίησης (Settings Alert Banner) απευθείας πάνω από τα κουμπιά ενεργειών («Έλεγχος Σύνδεσης», «Επαναφορά», «Αποθήκευση») με άμεση οπτική απεικόνιση επιτυχίας (πράσινο ✅) ή αποτυχίας (κόκκινο ❌) χωρίς να απαιτείται κύλιση της σελίδας προς τα πάνω (scroll up).

### Έκδοση 1.5.99
* **Ημερομηνία:** 31/08/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Διόρθωση σφάλματος επίλυσης και συγχρονισμού ενεργού Server URL στο Windows Service (`TmsAgentService`), ώστε να επιλέγει πάντα το κατάλληλο URL (Production / Test) βάσει του ενεργού περιβάλλοντος και να μην παραμένει σε κενό ή `localhost`.
  * Διασφάλιση αξιόπιστης εκτέλεσης αυτόματης αναβάθμισης συστήματος (Self-Upgrade) απευθείας από το Windows Service στο παρασκήνιο (Session 0) χωρίς να απαιτείται προηγούμενη σύνδεση χρήστη (login).
  * Προσθήκη αναλυτικής καταγραφής σφαλμάτων στο `agent.log` κατά την εκτέλεση του κύκλου του Windows Service.

### Έκδοση 1.5.98
* **Ημερομηνία:** 31/08/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Προσθήκη `ScrollViewer` στον Οδηγό Εγκατάστασης (`SetupWizardWindow`), εξασφαλίζοντας την εμφάνιση ScrollBar και την απρόσκοπτη εισαγωγή του API Key σε οθόνες με υψηλό DPI scaling ή χαμηλή ανάλυση.
  * Αύξηση των αρχικών διαστάσεων του παραθύρου εγκατάστασης (ύψος 560px, πλάτος 620px) και ενεργοποίηση της δυνατότητας αυξομείωσης μεγέθους (`ResizeMode="CanResizeWithGrip"`).
  * Δυναμική ανάγνωση και προβολή της τρέχουσας έκδοσης (`AgentVersionInfo.Version`) στην πλευρική στήλη του Setup Wizard.

### Έκδοση 1.5.97
* **Ημερομηνία:** 27/08/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Προσθήκη οπτικής επισήμανσης (μπλε πλαίσιο) του ενεργού περιβάλλοντος στον WPF Agent για εύκολο διαχωρισμό του Prod/Test.
  * Προσθήκη αμφίδρομου συγχρονισμού των API Keys (από τον Central στον Agent) κατά την επικοινωνία (check-in/update check).
  * Προσθήκη κουμπιών άμεσης παραγωγής τυχαίων API Keys (refresh button 🔄) μέσα στη φόρμα επεξεργασίας μηχανήματος στον Central Server.

### Έκδοση 1.5.96
* **Ημερομηνία:** 27/08/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Εισαγωγή ξεχωριστών API Keys και Server URLs για Παραγωγικό (Production) και Δοκιμαστικό (Test) περιβάλλον.
  * Δυνατότητα χειροκίνητης εισαγωγής/τροποποίησης των API Keys τόσο από την Web κονσόλα διαχείρισης (Central Server) όσο και απευθείας από το panel ρυθμίσεων του WPF Agent (για σύνδεση του ίδιου μηχανήματος και στους δύο servers).
  * Αυτόματη επιλογή του κατάλληλου API Key κατά την επικοινωνία (check-in, updates, logs, tickets κ.λπ.) ανάλογα με το ενεργό περιβάλλον (Production/Test) του Agent.

### Έκδοση 1.5.95
* **Ημερομηνία:** 27/08/2026
* **Αφορά:** Server
* **Περιγραφή:**
  * Κάθετη στοίχιση των κουμπιών ενεργειών (ρυθμίσεις ⚙️ και διαγραφή 🗑️) στις κάρτες πελατών για αποφυγή υπερχείλισης/overflow της σελίδας όταν η επωνυμία της εταιρείας είναι μεγάλη.
  * Σχεδίαση των κουμπιών ως ομοιόμορφα τετράγωνα 28x28px.

### Έκδοση 1.5.94
* **Ημερομηνία:** 26/08/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Προσθήκη ενδείξεων κατάστασης περιβάλλοντος (Testing/Production) και διορθώσεις στο redirect των περιβαλλόντων.

### Έκδοση 1.5.92
* **Ημερομηνία:** 26/08/2026
* **Αφορά:** Server
* **Περιγραφή:**
  * Διόρθωση σφάλματος επανεμφάνισης διαγραμμένων μηχανημάτων κατά το rebuild του Docker και προσθήκη cascade διαγραφής μηχανημάτων κατά τη διαγραφή πελάτη.

### Έκδοση 1.5.91
* **Ημερομηνία:** 26/08/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Προσθήκη αποστολής και απεικόνισης της διεύθυνσης Central Server (URL) του Agent στο UI του Central κάτω από το Alias.

### Έκδοση 1.5.90
* **Ημερομηνία:** 25/08/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Διόρθωση σφάλματος στην εξαγωγή αρχείων ZIP αναβάθμισης με αυτόματη αφαίρεση του κοινού αρχικού φακέλου (root folder stripping).
  * Προσθήκη αναλυτικής καταγραφής (logging) για την εξαγωγή όλων των DLL και RDLC αρχείων.
  * Ενσωμάτωση μηχανισμού συγχώνευσης XML config binding redirects αντί για ωμή αντικατάσταση.
  * Διόρθωση κωδικοποίησης (UTF-8 με BOM) στα μηνύματα καταγραφής (logs) για σωστή εμφάνιση των ελληνικών χαρακτήρων.

### Έκδοση 1.5.89
* **Ημερομηνία:** 20/07/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Προσθήκη μηχανισμού αυτόματης επαναπροσπάθειας (retry) έως 3 φορές με 2 δευτερόλεπτα καθυστέρηση στη λήψη των ZIP πακέτων αναβάθμισης.
  * Αυτόματη διαγραφή του μερικώς ληφθέντος αρχείου σε περίπτωση σφάλματος για την αποφυγή process locks ή κατεστραμμένων αρχείων.
  * Διόρθωση σφάλματος στον Server ώστε, αν η τελευταία κατάσταση αναβάθμισης ενός προφίλ ήταν `Failed`, να μην θεωρείται ενημερωμένο, επιτρέποντας στον χρήστη να ξαναπροσπαθήσει (retry) την αναβάθμιση.

### Έκδοση 1.5.88
* **Ημερομηνία:** 17/07/2026
* **Αφορά:** Server
* **Περιγραφή:**
  * Βελτιστοποίηση επιδόσεων κατά το check-in των Agents με on-demand (lazy) φόρτωση SQL scripts και in-memory caching των Script Identifiers για αποφυγή Regex parsing εκατοντάδων χιλιάδων γραμμών κώδικα ανά αίτημα.

### Έκδοση 1.5.87
* **Ημερομηνία:** 15/07/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Διορθώσεις στον επιλογέα διαδρομής αρχείου ρυθμίσεων (υποστήριξη υποφακέλων όπως `sup\Settings.ini` μέσω του `folderBrowserDummy.files`).
  * Αυτόματη καταγραφή και παρακολούθηση (`IsMonitored = true`) απομακρυσμένων ή μη ανιχνεύσιμων βάσεων δεδομένων που έχουν δηλωθεί στα εταιρικά προφίλ, ώστε να μην παρακάμπτεται η αναβάθμισή τους.

### Έκδοση 1.5.86
* **Ημερομηνία:** 15/07/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Προσθήκη δυνατότητας δυναμικού push/redirect του URL σύνδεσης των Agents από τον διακομιστή.
  * Εισαγωγή της παραμέτρου `AgentRedirectServerUrl` στο `appsettings.json` του Central Server.
  * Υλοποίηση αυτόματου εντοπισμού, ενημέρωσης του τοπικού `settings.json` και redirection της επικοινωνίας στον Agent (τόσο στην background υπηρεσία όσο και στο WPF Panel).

### Έκδοση 1.5.85
* **Ημερομηνία:** 15/07/2026
* **Αφορά:** Server & Client
* **Περιγραφή:**
  * Προσθήκη υποστήριξης για ανάγνωση Connection String από αρχεία ρυθμίσεων INI (ειδικά για τη δομή του settings.ini) με παράκαμψη σχολίων και σωστή διαχείριση προτεραιοτήτων (τιμές `:=` έναντι `:`).
  * Επέκταση του φίλτρου επιλογής αρχείων στον Web browser (Central Management) για την υποστήριξη αρχείων με κατάληξη `.ini`.
  * Υλοποίηση αυτόματης ανίχνευσης διαδρομής αρχείου ρυθμίσεων (ConfigFile) κατά την προσθήκη νέου μηχανήματος (αν ο χρήστης εισάγει διαδρομή στο πεδίο Connection String).

### Έκδοση 1.5.84
* **Ημερομηνία:** 14/07/2026
* **Αφορά:** Client
* **Περιγραφή:**
  * Παρακάμφθηκε η αυτόματη ανίχνευση τοπικών βάσεων δεδομένων SQL (`DiscoverLocalDatabases`) για Client-role Agents ώστε να αποφεύγονται timeouts και κολλήματα σύνδεσης/επικοινωνίας.
